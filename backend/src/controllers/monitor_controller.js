...
export async function getCustomPipelinesOverview(req, res) {
  try {
    ...
    try {
      ...
      const healthyPipelines = allDashboards.filter(d => {
        // Check if pipeline has recent healthy status
        if (d.health_series && d.health_series.length > 0) {
          const latestHealth = d.health_series[d.health_series.length - 1];
...
export async function deleteCustomPipelineMetrics(req, res) {
  try {
    ...
    const result = await customMonitorService.deleteCustomPipelineMetrics(id);
